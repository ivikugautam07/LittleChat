pipeline {
  ...
  environment {
    DOCKER_IMAGE_NAME = "LittleChatbackend"
    DOCKER_CONTAINER_NAME = "LittleChatbackend"
  ...
  stages {
    ...
    stage('creating .env file') {
      steps {
        script {
          withCredentials([string(credentialsId: 'LittleChat_DB_URL', variable: 'DB_URL'), string(credentialsId: 'LittleChat_JWT_SECRET_KEY', variable: 'JWT_SECRET_KEY')]) {
            def envContent = """
            NODE_ENV=development
            PORT=5000
            CORS_URL=https://LittleChat-indol.vercel.app
            SERVER_URL=https://LittleChatapi.bytenode.xyz
            DB_URL=${DB_URL}
            DB_NAME=LittleChat
            DB_MIN_POOL_SIZE=2
            DB_MAX_POOL_SIZE=5
            ...
            TOKEN_ISSUER=api.LittleChat.com
            TOKEN_AUDIENCE=LittleChat.com
